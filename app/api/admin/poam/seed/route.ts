/**
 * One-time POA&M seeding endpoint
 * Seeds all 14 not implemented controls as POA&M items
 * Admin only, can be called once after migration
 */
  ...
  try {
    ...
    for (const item of poamItems) {
      try {
        ...
        results.created.push(item.poamId)
      } catch (error: any) {
        if (error.code === "P2002") {
          results.skipped.push(item.poamId)
        } else {
          results.errors.push(`${item.poamId}: ${error.message}`)
        }
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: `POA&M seeding completed. Created: ${results.created.length}, Skipped: ${results.skipped.length}, Errors: ${results.errors.length}`,
      results,
    })
  } catch (error) {
    console.error("Error seeding POA&M items:", error)
    return NextResponse.json(
      { error: "Failed to seed POA&M items" },
      { status: 500 }
    )
  }
}
